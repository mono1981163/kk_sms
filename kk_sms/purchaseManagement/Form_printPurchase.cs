        ...
        private void button_ok_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (saveFileDialog_savePdf.ShowDialog() == DialogResult.OK)
                {
                    ...
                    //string query = "SELECT n.siireno, n.siirename, IFNULL((CASE WHEN (n.kuban = 0 OR n.kuban = 1) THEN SUM(n.kingaku) END), '0'), IFNULL((CASE WHEN (n.kuban = 0 OR n.kuban = 1) THEN SUM(n.kingaku) * (z.zei / 100) END), '0'), IFNULL((CASE WHEN n.kuban = 2 THEN SUM(n.kingaku) END), '0'), IFNULL((CASE WHEN n.kuban = 2 THEN SUM(n.kingaku) * (z.zei / 100) END), '0') FROM tbl_nyuko AS n, m_zei AS z WHERE nyukoday LIKE '" + date + "%' GROUP BY siireno";
                    string query = "SELECT n.siireno, n.siirename, SUM(n.kingaku), SUM(n.kingaku) * (z.zei / 100) AS tax, SUM(m.kingaku), SUM(m.kingaku) *(z.zei / 100) AS tax FROM tbl_nyuko AS n JOIN m_zei AS z LEFT JOIN tbl_nyuko AS m ON n.siireno = m.siireno WHERE(n.kuban = 1 OR n.kuban = 0) AND m.kuban = 2 AND n.nyukoday LIKE '" + date + "%' AND m.nyukoday LIKE '" + date + "%' GROUP BY n.siireno UNION SELECT n.siireno, n.siirename, SUM(n.kingaku), SUM(n.kingaku) * (z.zei / 100) AS tax, SUM(m.kingaku), SUM(m.kingaku) *(z.zei / 100) AS tax FROM tbl_nyuko AS n JOIN m_zei AS z RIGHT JOIN tbl_nyuko AS m ON n.siireno = m.siireno WHERE(n.kuban = 1 OR n.kuban = 0) AND m.kuban = 2 AND n.nyukoday LIKE '" + date + "%' AND m.nyukoday LIKE '" + date + "%' GROUP BY n.siireno";
